...
export default async function LeagueTablePage() {
  ...
  const maxTopPlaces = Math.max(...competitions.map((c) => c.topPlaces), 5);
  const hasData = leagueTable.length > 0;
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              ...
              {competitions.length !== 1 ? "s" : ""} played
            </p>
          </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
          ...
          <div className="flex flex-wrap gap-3">
            {Array.from({ length: maxTopPlaces }, (_, i) => i + 1).map(
              (place) => (
                <span
                  key={place}
                  className="text-sm text-green-700 bg-green-100 px-3 py-1 rounded-full"
                >
                  {ordinal(place)} = {pointsForPlace(place, maxTopPlaces)} pts
                </span>
              )
            )}
          </div>
